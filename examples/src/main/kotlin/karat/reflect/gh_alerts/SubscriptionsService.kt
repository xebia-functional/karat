...
@initial object SubsInitial: SubscriptionsAction {
  context(ReflectedModule) override fun execute(): KFormula = no(theService.db)
}

data class SubsPut(val user: KArg<User>, val repo: KArg<Repo>): SubscriptionsAction {
  context(ReflectedModule) override fun execute(): KFormula = and {
    + (next(theService.db) `==` current(theService.db) + (user to repo))
    + forOne<StartListen> { m -> and {
      + (m.subsRepo `==` repo)
...
data class SubsDelete(val user: KArg<User>, val repo: KArg<Repo>): SubscriptionsAction {
  context(ReflectedModule) override fun execute(): KFormula = and {
    + (next(theService.db) `==` current(theService.db) - (user to repo))
    + forNo<User> { u -> u `in` theService.db[u] }.ifThen(
      ifTrue = forOne<StopListen> { m -> and {
        + (m.subsRepo `==` repo)
        + theService.subscriptionsTopic.send(m)
      } },
...
object SubsEvent: SubscriptionsAction {
  context(ReflectedModule) override fun execute(): KFormula = and {
    + stays(theService.db)
    + forSome<EventsMessage> { e -> and {
      + theService.eventsTopic.consume(e)
      + forOne<NotificationsMessage> { n -> and {
