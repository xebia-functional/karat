...
open class KModuleBuilder: ReflectedModule {
  ...
  private fun reflectClassAsSig(type: KType) {
    ...
    val attribs = listOfNotNull(
      (Attr.ABSTRACT)?.takeIf { klass.hasAnnotation<com.`47deg`.karat.abstract>() },
      (Attr.ONE)?.takeIf { klass.hasAnnotation<com.`47deg`.karat.one>() || klass.hasAnnotation<com.`47deg`.karat.element>() || klass.objectInstance != null },
      (Attr.VARIABLE)?.takeIf { klass.hasAnnotation<com.`47deg`.karat.variable>() }
    )
    val isSubset = klass.hasAnnotation<com.`47deg`.karat.subset>() || klass.hasAnnotation<com.`47deg`.karat.element>()
  ...
  private fun reflectClassFields(reflectAll: Boolean, type: KType) {
    ...
    klass.declaredMemberProperties
      ...
      .filter {
        reflectAll || it.hasAnnotation<com.`47deg`.karat.reflect>()
      }
      .forEach { property ->
        ...
        val sigTy: KSet<*>? = when {
          ...
          ret.klass?.isSubclassOf(Set::class) == true ->
            findSet(ret.arguments.firstOrNull()?.type)?.let {
              when {
                property.hasAnnotation<com.`47deg`.karat.lone>() -> loneOf(it)
                property.hasAnnotation<com.`47deg`.karat.one>() -> oneOf(it)
                property.hasAnnotation<com.`47deg`.karat.some>() -> someOf(it)
                else -> setOf(it)
              }
            }
        ...
        when (sigTy) {
          null -> throw IllegalArgumentException("cannot reflect type $ret")
            ...
            val newProp =
              if (property is KMutableProperty<*> || property.hasAnnotation<com.`47deg`.karat.variable>())
  ...
  private fun reflectProperty(property: KProperty<*>) {
    ...
    val sigTy: Pair<List<Attr>, KSig<*>>? = when {
      ...
      ret.klass?.isSubclassOf(Set::class) == true ->
        findSet(ret.arguments.firstOrNull()?.type)?.let {
          when {
            property.hasAnnotation<com.`47deg`.karat.lone>() -> listOf(Attr.LONE) to it
            property.hasAnnotation<com.`47deg`.karat.one>() -> listOf(Attr.ONE) to it
            property.hasAnnotation<com.`47deg`.karat.some>() -> emptyList<Attr>() to it
